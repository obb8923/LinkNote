...
import { GraphLink, GraphNode } from '@/shared/types/graphType';
...
export const AddRelationScreen = () => {
  ...
  const addLink = useGraphStore((state) => state.addLink);
  ...
  const handleSave = async () => {
    try {
      // 필수 입력값 검증
      if (!sourcePersonId) {
        Alert.alert('알림', '첫 번째 지인을 선택해주세요.');
        return;
      }

      if (!targetPersonId) {
        Alert.alert('알림', '두 번째 지인을 선택해주세요.');
        return;
      }

      if (relationScore === null || relationScore === undefined) {
        Alert.alert('알림', '관계 점수를 선택해주세요.');
        return;
      }
      ...
      if (!sourceNode) {
        ...
        Alert.alert('오류', '첫 번째 지인의 노드를 찾을 수 없습니다.');
        return;
      }

      if (!targetNode) {
        console.error('Target node not found for personId:', targetPersonId);
        Alert.alert('오류', '두 번째 지인의 노드를 찾을 수 없습니다.');
        return;
      }
      ...
      // graphStore에도 링크 추가 (그래프 시각화용)
      const newLink: GraphLink = {
        id: uuid.v4() as string,
        source: sourceNode,
        target: targetNode,
        type: relationDescription.trim() || '',
        strength: relationScore as RelationStrength,
      };
      ...
      Alert.alert('오류', '관계 저장 중 오류가 발생했습니다.');
    }
  };
      ...
      >
        ...
        >
            ...
            <View className="w-full mb-6">
                <Text text="두 지인 선택" type="body3" className="text-neutral-600 font-semibold mb-2" />
                ...
                <View className="w-[40%]">
                  <SearchableList
                    ...
                    placeholder={sourcePersonId ? getPersonName(sourcePersonId) : '지인 선택'}
                ...
                <View className="w-[20%] items-center justify-center">
                  ...
                  >
                    <Text text={getArrowSymbol(arrowDirection)} type="title2" className="text-black" />
                  </TouchableOpacity>
                </View>
                {/* 지인 2 선택 */}
                <View className="w-[40%]">
                  <SearchableList
                    ...
                    placeholder={targetPersonId ? getPersonName(targetPersonId) : '지인 선택'}
            ...
            <View className="w-full mb-6">
                ...
                <Text text="관계 점수" type="body3" className="text-neutral-600 font-semibold" />
                <Text text={relationScore ? relationScore.toString() + '점': '0점'} type="body3" className="text-neutral-900 font-semibold ml-2" />
                </View>
                <View className="w-full bg-blue-chip-50 py-4 rounded-xl flex-row items-center justify-evenly">
                    {([1, 2, 3, 4, 5] as RelationStrength[]).map((score) => (
                        ...
                        >
                            <HeartIcon
                                width={24}
                                height={24}
                                color={relationScore && score <= relationScore ? COLORS.BLUE_CHIP_500 : COLORS.BLUE_CHIP_300}
                                fill={relationScore && score <= relationScore ? COLORS.BLUE_CHIP_500 : 'none'}
                            />
                        </TouchableOpacity>
                    ))}
                </View>
          ...
          {/* 관계 설명 입력 (선택사항) */}
          <View className="flex-1 border-t border-gray-300 pt-6">
            <Text text="관계 설명 (선택)" type="body3" className="text-neutral-600 font-semibold mb-2" />
            <TextInput
              ...
              className="px-0 py-3 text-black"
              style={{
